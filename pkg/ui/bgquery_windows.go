...
// OSC 11. termenv (which lipgloss.HasDarkBackground ultimately calls) does
// not implement this on Windows — its backgroundColor() is hardcoded to
// black (see termenv_windows.go), so auto-detection always reports "dark"
// there regardless of the actual console theme (bv-128). This is bv's own
// best-effort fallback, and is only attempted inside Windows Terminal
// (WT_SESSION set), the one Windows console host known to answer OSC 11;
// legacy conhost does not, so ok is false there and callers keep the
// existing (always-dark) result.
func platformHasDarkBackground() (isDark bool, ok bool) {
	...
	case <-time.After(windowsBackgroundQueryTimeout):
		...
		// whatever reads stdin next. This is why the query must run once at
		// startup, before bubbletea takes stdin (mirrored by SetThemeOverride
		// being called before any TUI model starts).
		return false, false
	}
}
