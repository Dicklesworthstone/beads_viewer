...
type CommandHelpers struct {
	ClaimTop      string `json:"claim_top"`      // CI=1 <beads-cli> update <id> --status in_progress --json
	ShowTop       string `json:"show_top"`       // CI=1 <beads-cli> show <id> --json
	ListReady     string `json:"list_ready"`     // CI=1 <beads-cli> ready --json
	ListBlocked   string `json:"list_blocked"`   // CI=1 <beads-cli> blocked --json
...
type TrackRecommendationGroup struct {
	...
	ClaimCommand    string           `json:"claim_command,omitempty"` // CI=1 <beads-cli> update <top_pick_id> --status in_progress --json
